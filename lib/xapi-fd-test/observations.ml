...
let pp ppf =
  Fmt.(
    record
      ...
      ; field "data" (fun t -> t.data) string
      ]
  )
    ppf
...
let observe_ro write ~f kind expected =
  ...
  let g fd_opt =
    ...
    |> Option.fold ~none:None ~some:(fun fd ->
           do_write write written expected off (as_writable_opt fd)
       )
  in
...
let observe_wo read ~f kind =
  ...
  let g fd_opt =
    fd_opt
    |> Option.fold ~none:None ~some:(fun fd ->
           do_read read rd_buf (as_readable_opt fd)
       )
  in
